...
$filtro = $_POST['filtro_excel_finalizado'];

if (empty($_POST['filtro_excel_finalizado'])) {
  $filtro = "0";
}
...
if($filtro == "1" || $filtro == "0" ){
  ...
  header('Content-Disposition: attachment; filename=reporte-servicios-finalizados.xls');

  $excel = new PHPExcel();
  $excel->getProperties()->setCreator('Taxisya')->setLastModifiedBy('Taxisya')->setTitle('Reporte');
  $excel->setActiveSheetIndex(0);
  $pagina = $excel->getActiveSheet();
  $pagina->setTitle('Servicios-finalizados');
  $filas = $consulta->reporterExcelFinalizados();
  ...
  $pagina = $excel->getActiveSheet();
  $pagina->setTitle('Servicios-finalizados');
  $filas = $consulta->reporterExcelFinalizadosLimit($filtro);
