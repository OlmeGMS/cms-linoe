...
$filtro = $_POST['filtro_excel_operador'];

if (empty($_POST['filtro_excel_operador'])) {
  $filtro = "0";
}
...
if($filtro == "1" || $filtro == "0" ){
  ...
  header('Content-Disposition: attachment; filename=reporte-servicios-cancelados-operador.xls');

  $excel = new PHPExcel();
  $excel->getProperties()->setCreator('Taxisya')->setLastModifiedBy('Taxisya')->setTitle('Reporte');
  $excel->setActiveSheetIndex(0);
  $pagina = $excel->getActiveSheet();
  $pagina->setTitle('Servicios-cancelados-operador');
  $filas = $consulta->reporterExcelCanceladoOperador();
  ...
  $pagina = $excel->getActiveSheet();
  $pagina->setTitle('Servicios-cancelados-operador');
  $filas = $consulta->reporterExcelCanceladosOperadorLimit($filtro);
