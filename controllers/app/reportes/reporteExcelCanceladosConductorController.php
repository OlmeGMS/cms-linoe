...
$filtro = $_POST['filtro_excel_conductor'];

if (empty($_POST['filtro_excel_conductor'])) {
  $filtro = "0";
}
...
if($filtro == "1" || $filtro == "0" ){
  ...
  header('Content-Disposition: attachment; filename=reporte-servicios-cancelados-conductor.xls');

  $excel = new PHPExcel();
  $excel->getProperties()->setCreator('Taxisya')->setLastModifiedBy('Taxisya')->setTitle('Reporte');
  $excel->setActiveSheetIndex(0);
  $pagina = $excel->getActiveSheet();
  $pagina->setTitle('Servicios-cancelados-conductor');
  $filas = $consulta->reporterExcelCanceladoConductor();
  ...
  $pagina = $excel->getActiveSheet();
  $pagina->setTitle('Servicios-cancelados-conductor');
  $filas = $consulta->reporterExcelCanceladosConductorLimit($filtro);
